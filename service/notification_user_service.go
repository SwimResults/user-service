...
func RegisterNotificationUser(token string, device model.Device, settings model.NotificationSettings, pushService string, user *model.User) (model.NotificationUser, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
